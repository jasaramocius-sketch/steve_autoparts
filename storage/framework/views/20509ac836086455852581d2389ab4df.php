<?php $__env->startSection('page-id', 'user-reviews-page'); ?>
<?php $__env->startSection('page-class', 'user-reviews-page'); ?>
...
<div class="view-reviews-filter d-flex align-items-center justify-content-between mb-3 gap-2 flex-wrap">
    <form method="GET" action="<?php echo e(route('user.reviews')); ?>" class="d-flex gap-2 align-items-center" id="reviewFilterForm">
    ...
    <form method="GET" action="<?php echo e(route('user.reviews')); ?>" id="statusFilterForm">
...
<div class="table-responsive">
    <table class="table table--custom table--responsive-lg table-hover">
        ...
        <?php $__empty_1 = true; $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr id="review-row-<?php echo e(md5($item['product_slug'])); ?>">
                <td data-label="Product">
                    <div class="d-flex align-items-center gap-2">
                        <?php if($item['product_image']): ?>
                            <img src="<?php echo e(asset('assets/images/thumbnails/' . $item['product_image'])); ?>" alt="" style="width:40px;height:40px;object-fit:cover;border-radius:6px;">
                        <?php else: ?>
                        ...
                        <div class="d-flex  justify-content-center gap-1">
                            <?php for($i = 0; $i < 5; $i++): ?>
                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="13" viewBox="0 0 17 16" fill="none">
                                    <path d="M8.5 0.5L10.4084 6.37336L16.584 6.37336L11.5878 10.0033L13.4962 15.8766L8.5 12.2467L3.50383 15.8766L5.41219 10.0033L0.416019 6.37336L6.59163 6.37336L8.5 0.5Z" fill="<?php echo e($i < $item['rating'] ? '#EEAE0B' : '#E2E8F0'); ?>" />
                                </svg>
                            <?php endfor; ?>
                        </div>
                    <?php endif; ?>
                    ...
                    <?php if($item['status'] === 'pending'): ?>
                        <div class="action-buttons" style="justify-content:flex-end;">
                            <button type="button" class="action-btn btn-invoice review-action-btn" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Write Review"
                                data-slug="<?php echo e($item['product_slug']); ?>"
                                data-name="<?php echo e($item['product_name']); ?>"
                                data-image="<?php echo e($item['product_image'] ? asset('assets/images/thumbnails/' . $item['product_image']) : asset('assets/images/placeholder.png')); ?>"
                                data-status="<?php echo e($item['status']); ?>"
                                data-review-id=""
                                data-rating="<?php echo e($item['rating']); ?>"
                                data-text=""
                                data-images="[]"
                                data-date="">
                    ...
                    <?php else: ?>
                        <div class="action-buttons" style="justify-content:flex-end;">
                            <button type="button" class="action-btn btn-view review-view-btn" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="View Review"
                                ...
                                data-image="<?php echo e($item['product_image'] ? asset('assets/images/thumbnails/' . $item['product_image']) : asset('assets/images/placeholder.png')); ?>"
                                data-rating="<?php echo e($item['rating']); ?>"
                                data-text="<?php echo e(addslashes($item['text'] ?? '')); ?>"
                                data-images="<?php echo htmlentities(json_encode($item['images'] ?? [])); ?>"
                            ...
                            <button type="button" class="action-btn btn-invoice review-edit-btn" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Edit Review"
                                ...
                                data-name="<?php echo e($item['product_name']); ?>"
                                data-image="<?php echo e($item['product_image'] ? asset('assets/images/thumbnails/' . $item['product_image']) : asset('assets/images/placeholder.png')); ?>"
                                data-review-id="<?php echo e($item['review_id'] ?? ''); ?>"
                                data-rating="<?php echo e($item['rating']); ?>"
                                data-text="<?php echo e(addslashes($item['text'] ?? '')); ?>"
                                data-images="<?php echo htmlentities(json_encode($item['images'] ?? [])); ?>">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
<?php if(method_exists($items, 'links')): ?>
    <div class="pagination-wrapper mt-4">
        <?php echo e($items->links('vendor.pagination.gs-pagination')); ?>
        ...
        <div class="modal-body gry-bg px-3 pt-3">
          ...
          <div class="form-group mb-3">
            ...
            <div class="star-picker d-flex align-items-center gap-1 mt-1" id="reviewModalStarPicker">
              <?php for($i = 1; $i <= 5; $i++): ?>
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="21" viewBox="0 0 17 16" fill="none" data-rating="<?php echo e($i); ?>" class="review-star-pick" style="cursor:pointer;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.review-action-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            document.getElementById('reviewProductLink2').href = '<?php echo e(url("/product")); ?>/' + btn.dataset.slug;
    ...
    document.querySelectorAll('.review-view-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            document.getElementById('reviewProductLink').href = '<?php echo e(url("/product")); ?>/' + btn.dataset.slug;
            ...
            try {
                ...
                existingImages.forEach(function(imgPath) {
                    ...
                    a.setAttribute('data-review-img', '<?php echo e(asset("/")); ?>' + imgPath);
                    a.className = 'review-lightbox-trigger';
                    var img = document.createElement('img');
                    img.src = '<?php echo e(asset("/")); ?>' + imgPath;
    ...
    document.querySelectorAll('.review-delete-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            fetch('<?php echo e(url("/product")); ?>/' + slug + '/review/' + reviewId, {
                method: 'DELETE',
                headers: {
    ...
    submitBtn.addEventListener('click', function() {
        ...
        var url = currentReviewId
            ? '<?php echo e(url("/product")); ?>/' + currentSlug + '/review/' + currentReviewId
            : '<?php echo e(url("/product")); ?>/' + currentSlug + '/review';
...
<?php echo $__env->make('user.layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/stautoparts/resources/views/user/reviews.blade.php ENDPATH**/ ?>