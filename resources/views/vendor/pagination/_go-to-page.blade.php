@if ($paginator->lastPage() > 1)
    <form method="GET" action="{{ $paginator->path() }}" class="d-flex align-items-center gap-2 Go-to-page">
        ...
        <input type="number" id="gs-go-to-page-number" name="page" class="form-control form-control-sm" style="width: 50px;" min="1" max="{{ $paginator->lastPage() }}" value="{{ $paginator->currentPage() }}" aria-label="Page number">
