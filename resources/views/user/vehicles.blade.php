...
<div class="card mb-3">
    <div class="card-body d-flex justify-content-between align-items-center">
        <div>
            ...
            @if($vehicle->engine)
            <p class="mb-0 text-muted">Engine: {{ $vehicle->engine }}</p>
            @endif
        </div>
        <div class="d-flex gap-2">
            <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editVehicleModal{{ $vehicle->id }}">Edit</button>
            <form action="{{ route('user.vehicles.destroy', $vehicle->id) }}" method="POST" onsubmit="return confirm('Remove this vehicle?')">
                @csrf @method('DELETE')
    ...
    <div class="modal-dialog">
        <form action="{{ route('user.vehicles.update', $vehicle->id) }}" method="POST">
            ...
            <div class="modal-content">
                <div class="modal-header"><h5 class="modal-title">Edit Vehicle</h5><button class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Year</label>
                        <input type="text" name="year" class="form-control" value="{{ $vehicle->year }}" required>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="engine" class="form-control" value="{{ $vehicle->engine }}">
                    </div>
                </div>
...
<div class="modal fade" id="addVehicleModal" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('user.vehicles.store') }}" method="POST">
            @csrf
            <div class="modal-content">
                <div class="modal-header"><h5 class="modal-title">Add Vehicle</h5><button class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Year</label>
