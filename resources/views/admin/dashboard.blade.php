        ...
        <div class="card border-0 shadow-sm h-100">
            ...
            <div class="card-body">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <span class="text-capitalize fw-medium">{{ $status->status }}</span>
                    <div class="d-flex align-items-center gap-3">
                        <div class="progress" style="width: 200px; height: 8px;">
                            <div class="progress-bar" role="progressbar"
                                 style="width: {{ $totalOrders > 0 ? ($status->count / $totalOrders) * 100 : 0 }}%;"
                                 aria-valuenow="{{ $status->count }}" aria-valuemin="0" aria-valuemax="{{ $totalOrders }}">
                            </div>
                        </div>
    ...
    <div class="col-md-6">
        <div class="card border-0 shadow-sm h-100">
            ...
            <div class="card-body">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <span class="fw-medium">{{ $month->month }}</span>
                    <div class="d-flex align-items-center gap-3">
                        <div class="progress" style="width: 200px; height: 8px;">
                            <div class="progress-bar bg-success" role="progressbar"
                                 style="width: {{ $monthlyRevenue->max('total') > 0 ? ($month->total / $monthlyRevenue->max('total')) * 100 : 0 }}%;"
                                 aria-valuenow="{{ $month->total }}" aria-valuemin="0" aria-valuemax="{{ $monthlyRevenue->max('total') }}">
                            </div>
                        </div>
                        <span class="fw-bold">{{ currency_format($month->total) }}</span>
                    </div>
                </div>
                @empty
