@extends('layouts.app')
@section('title', 'Vendor Login - StAutoparts')
@section('content')
      ...
      <div class="col-lg-6 mx-auto reg-area">
        <div class="reg-content">
          ...
          <form action="{{ route('vendor.login.post') }}" method="POST">
            ...
            <button type="submit" class="template-btn btn-forms" style="background-color: var(--primary); border-color: var(--primary); color: #fff; height: 50px; font-weight: 500; border-radius: 4px;">Login</button>
            <p class="text-center login-or">Or</p>
            <br>
            <p class="login-redirect">Don't have an account? <span><a href="{{ route('vendor.register') }}">Create New Account</a></span></p>
          </form>
        </div>
      </div>
    </div>
