...
@section('page-id', 'user-order-details-page')
@section('page-class', 'user-order-details-page')
...
<div class="ud-page-title-box" style="justify-content:space-between;">
    <div>
        <h3 style="margin:0;">Order# {{ $order->order_number ?? '#' . $order->id }}<span style="color:var(--primary);font-weight:400;font-size:24px;margin-left:8px;">[{{ $currentStatus }}]</span></h3>
        <p style="margin:4px 0 0 0;color:#796866;">Order Date {{ optional($order->created_at)->format('d-M-Y') }}</p>
    </div>
    <div style="display:flex;gap:12px;align-items:flex-start;">
        <a href="{{ route('user.orders.invoice', $order->id) }}" target="_blank" class="template-btn outline-btn lg-btn steve-btn steve-btn-sm">
...
<div class="gs-checkout-wrapper"    >
    <div class="checkout-step-wrapper" style="margin-bottom:40px;">
        ...
        @endphp
        <span style="position:absolute;top:31%;z-index:2;width:{{ $statusPercent }}%;border-top:2px solid var(--primary);left:50px;transition:width 0.5s;max-width:calc(100% - 100px);"></span>
        ...
        @foreach($statusSteps as $index => $step)
            ...
            @endphp
            <div class="single-step {{ $isActive ? 'active' : '' }}">
                <div class="step-btn">{{ $index + 1 }}</div>
                <div class="step-txt" style="{{ $isActive ? 'color:var(--primary);' : '' }}">{{ $step }}</div>
            </div>
        @endforeach
    </div>
</div>
        ...
        @if(!empty($shipping))
            <div style="display:flex;flex-direction:column;gap:8px;">
                ...
                <div style="display:flex;align-items:center;gap:8px;">
                    ...
                    <p style="margin:0;font-size:16px;color:#4c3533;">{{ $shipping['name'] }}</p>
                </div>
                @endif
                <div style="display:flex;align-items:center;gap:8px;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#796866" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                    <p style="margin:0;font-size:16px;color:#4c3533;">
                        {{ $shipping['address'] === "old('address')" ? 'Street Address' : $shipping['address'] }},
                        {{ $shipping['city'] ?? '' }}{{ !empty($shipping['city']) && !empty($shipping['zip_code']) ? ',' : '' }}
                        {{ $shipping['zip_code'] ?? '' }}
                    </p>
                </div>
                ...
                <div style="display:flex;align-items:center;gap:8px;">
                    ...
                    <p style="margin:0;font-size:16px;color:#4c3533;">{{ $shipping['phone'] }}</p>
                </div>
                @endif
                ...
                <div style="display:flex;align-items:center;gap:8px;">
                    ...
                    <p style="margin:0;font-size:16px;color:#4c3533;">{{ $shipping['email'] }}</p>
                </div>
                @endif
            </div>
        @else
...
<div class="row g-4 mt-2">
    <div class="col-lg-6">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
            ...
            <p style="margin:0;font-size:16px;color:#4c3533;">
                <span style="font-weight:500;">Tax :</span> {{ currency_format($taxAmount) }}
            </p>
            <p style="margin:0;font-size:16px;color:#4c3533;">
                <span style="font-weight:500;">Paid Amount :</span> {{ currency_format($totalAmount) }}
            </p>
            <p style="margin:0;font-size:16px;color:#4c3533;">
                <span style="font-weight:500;">Payment Method :</span> {{ $order->payment_method ? ucfirst(str_replace('_', ' ', $order->payment_method)) : 'Cash On Delivery' }}
            </p>
        </div>
    </div>
    ...
    <div class="col-lg-6">
        ...
        <p style="font-size:16px;color:#4c3533;">{{ $order->delivery_type ?? 'Free Shipping' }}</p>
    </div>
</div>
        ...
        <table class="table">
            ...
            <tbody class="tbody-product">
                ...
                <tr>
                    <td class="td-product-name">
                        <ul>
                            <li>
                                ...
                                @if($item->product)
                                    <a href="{{ route('product', $item->product->slug ?? '') }}" target="_blank" class="text-reset fw-600">
                                        {{ $item->product->name }}
                                    </a>
                                @else
                    ...
                    <td><span>{{ currency_format($item->price ?? 0) }}</span></td>
                    <td>
