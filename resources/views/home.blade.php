@extends('layouts.app')
@section('title', config('app.name', 'StAutoparts'))
@section('content')
                    ...
                    <div class="col-12 col-md-6 col-lg-6">
                        <div class="hero-content">
                            ...
                            @if($heroSection->button_text && $heroSection->button_url)
                                <a class="template-btn hero-shop-now-btn wow-replaced" data-wow-delay=".3s" href="{{ $heroSection->button_url }}">
                ...
                <div class="row align-items-center">
                    <div class="col-12 col-md-6 col-lg-6">
                        <div class="hero-content">
                            ...
                            <a class="template-btn hero-shop-now-btn wow-replaced" data-wow-delay=".3s" href="{{ route('shop') }}" style="background-color: var(--hov-primary); color: white; padding: 20px 30px; border-radius: 4px; display: inline-block; font-weight: 600;">
                                Shop Now
                            </a>
                        </div>
                    </div>
...
<div class="gs-cate-section">
    <div class="container wow-replaced">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h3>{{ $sections->get('categories_heading')?->title ??  'All Categories' }}</h3>
            <div class="slider-nav">
                <button class="cate-prev">
                    ...
                    <div class="gs-single-cat">
                        <div class="gs-single-cat">
                            <div class="category-image">
                                ...
                                @if($category['image'])
                                    {!! imgTag('assets/images/categories/'.$category['image'], $category['name']) !!}
                                @else
                                    <div class="category-letter-placeholder">
                        ...
                        @else
                            <div class="bg-light p-5 text-center" style="min-height: 300px; display: flex; align-items: center; justify-content: center; border-radius: 4px;">
                                <div>
                                    <h5>{{ $banner->title }}</h5>
                                    <p>{{ $banner->subtitle }}</p>
                                    <button class="btn btn-primary">{{ $banner->button_text ??  'Learn More' }}</button>
                                </div>
                            </div>
                        @endif
                    </a>
        ...
        <div class="row mb-36 justify-content-center">
            <div class="col-12">
                <div class="gs-title-box text-center">
                    <h2 class="title wow-replaced">{{ $sections->get('explore_products')?->title ??  'Featured Products' }}</h2>
                </div>
                <!-- product nav -->
                <ul class="nav explore-tab-navbar wow-replaced" data-wow-delay=".1s" id="myTab" role="tablist">
                    @foreach($exploreTabs as $i => $tab)
                    <li class="nav-item" role="presentation">
                        <button class="nav-link {{ $i === 0 ? 'active' : '' }}" id="ex-product-{{ $i + 1 }}" data-bs-toggle="tab" data-bs-target="#ex-product-{{ $i + 1 }}-pane" type="button" role="tab" aria-controls="ex-product-{{ $i + 1 }}-pane" aria-selected="{{ $i === 0 ? 'true' : 'false' }}">{{ $tab['label'] }}</button>
                    </li>
                    @endforeach
                </ul>
            </div>
                        ...
                        <div class="single-product">
                            <div class="img-wrapper">
                                ...
                                <form action="{{ route('wishlist.add') }}" method="POST" class="wishlist-form">
                                    ...
                                    <button type="button"
                                        class="add-to-wishlist-btn wishlist-btn border-0 bg-transparent">
                                ...
                                <div class="add-to-cart">
                                    ...
                                    <form action="{{ route('cart.add') }}" method="POST" class="add-cart-form d-inline">
                                        @csrf
                                        <input type="hidden" name="product_id" value="{{ $product['id'] }}">
                ...
                <div class="deal-of-day-wrapper">
                    <div class="deal-of-day-content">
                        ...
                        <div class="countdown-wrapper flex-wrap " id="countdown">
                            ...
                            <a href="{{ $deal?->button_url ?? route('shop') }}" class="template-btn w-100 wow-replaced" data-wow-delay=".7s">{{ $deal?->button_text ??  'Shop Now' }}</a>
                        </div>
                    </div>
                </div>
            </div>
            @php $dealRelPath = $deal?->image ? 'assets/images/home/'.$deal->image : 'assets/images/1730873933bANNER2png.png'; @endphp
...
<section class="gs-explore-product-section bg-white">
    <div class="container">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div class="gs-title-box">
                <h2 class="title mb-0">{{ $sections->get('featured_products_heading')?->title ??  'Featured Products' }}</h2>
            </div>
        ...
        <div class="product-cards-slider">
            ...
            <div class="col-md-6 col-lg-4 col-xl-3">@include('partials.product-card', ['product' => $product])</div>
            @endforeach
...
<section class="gs-explore-product-section">
    <div class="container">
        ...
        <div class="best-selling-nav">
                <button class="best-selling-prev">
                    <i class="fas fa-chevron-left"></i>
                </button>

                <button class="best-selling-next">
                    <i class="fas fa-chevron-right"></i>
                </button>
            </div>
        <div class="best-selling product-cards-slider">
        ...
        <div class="row gy-5 latest-post-area m-0">
            ...
            <div class="col-lg-6 posts-area wow-replaced" data-wow-delay=".2s">
                <a href="{{ route('blog.show', $post->slug) }}" class="single-post h2-single-post">
                    <div class="post-img">
                        {!! imgTag('assets/images/blogs/' . ($post->image ?? 'placeholder.jpg'), $post->title) !!}
                    </div>
